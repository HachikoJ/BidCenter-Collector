...
默认详情间隔为 `5000ms`、默认并发数为 `3`。并发通道不会同时启动详情，每次打开前会随机错峰 `0.1-3` 秒；该错峰时间独立于用户设置的详情间隔。暂停任务后修改详情间隔会在点击“继续”时应用到当前断点任务。用户自行调高并发或缩短间隔会增加触发平台限制的风险。插件检测到滑块验证后会保存断点并暂停，必须由用户在官网页面手动完成；插件不会尝试绕过验证、登录、会员或访问控制。
...
- 详情间隔默认调整为 `5000ms`，暂停后修改间隔会在点击“继续”时写入当前任务。
