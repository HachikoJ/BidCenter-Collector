...
默认详情基础间隔为 `5000ms`、默认并发数为 `3`。每条详情会在基础间隔上随机偏移 `-2` 至 `+2` 秒，低于插件下限时按 `200ms` 处理。并发通道不会同时启动详情，每次打开前还会随机错峰 `0.1-3` 秒；启动错峰独立于随机详情间隔。暂停任务后修改基础间隔会在点击“继续”时应用到当前断点任务。用户自行调高并发或缩短间隔会增加触发平台限制的风险。插件检测到滑块验证后会保存断点并暂停，必须由用户在官网页面手动完成；插件不会尝试绕过验证、登录、会员或访问控制。
